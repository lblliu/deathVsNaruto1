...
cc.Class({
    ...
    shotSword2() {//影风车回调
        g_SkillManger.doSkill(1005, 2);
    },
    shotSword1() {//手里剑
        g_SkillManger.doSkill(1006, 2);
    },
    ...
    doHurt(other) {
        ...
        if (other.tag == 11020) {
            let parentN = other.node.parent.parent;
            g_Object.getRole1().stopSkill("luoxuanwan2");
            attackID = parentN.uid;
            skillID = 1;
            this.hurt = 10;
        }
    ...
    doAttack() {
        if (g_defind.lock2 && !this.damageLock) {
            g_defind.lock2 = false;
    ...
    doJump() {//跳
        if (!g_defind.lock2 || this.damageLock || this.jumpNum > 1 || this.jumpAttackFlag == 2) {
            return;
        } else {
            if (this.jumpNum == 0) {
                this.jumpNum = 1;
    ...
    doDefense() {//防御
        if ((g_defind.lock2 || this.defenseFlag == 2) && !this.damageLock) {
    ...
    changeHp(skillID, number) {//扣血
        ...
        this.hp -= hurt;
        this.hpRatio.progress = 1 - (100 - this.hp) / 100;
        if (this.hp <= 0) {
            if (this.hpNum > 0) {
                this.hp = 100;
                this.hpNum--;
    ...
    changeSkillState(state, cb) {
        ...
        if (g_defind.lock2 || state == "damage") {
            g_defind.lock2 = false;
    ...
    doMove() {
        if (this.isDie || !g_defind.lock2 || this.damageLock) {
            return;
        }
        let x = 0;
        if (this.keyCtrl & 2) {
        ...
        if (x == 0 && this.m_State == "run") {
            if (cc.audioEngine.getState(this.stateRunEffect) == 1) {
                cc.audioEngine.stopEffect(this.stateRunEffect);
                this.stateRunEffect = 0;
            }
            this.changeState("standby");
        }
    },
    ...
    onCollisionExit(other, self) {
        if (Math.floor(self.tag / 1000) == 1) {
            if (other.tag == 1 || other.tag == 5) {
                this.collision &= ~(self.tag - 1000);
                if (self.tag == 1002 && (other.tag == 1 || other.tag == 5)) {
                    this.conuntNum--;
                    if (this.conuntNum == 0) {
    ...
    onLoad() {
        ...
        this.node.uid = g_defind.roleList[1];
        this.ANIM = this.node.getComponent(cc.Animation);
    },

    start() {
        this.stateRunEffect = 0;//记录跑步时音效状态
        this.flagAttack = false;
        this.jumpNum = 0;//跳跃次数
        this.damageLock = false;//受击状态僵直锁
        this.stopFlag = false;
        this.jumpAttackFlag = 1;//跳踢技能锁
        this.seriousinjuryFlag = true;
        this.conuntNum = 0;
        this.defenseFlag = 1;
        this.keyCtrl = 0;
        ...
        this.angerNum = 0;
        this.speed = 5;
        this.m_ID = g_defind.roleList[1];
        this.m_Shape = 1001;
        this.attackNum = 0;
        this.m_State = "standby";
        this.time = 0;
        this.effectLayer = cc.find("Canvas/effectLayer");
        this.hpPro = cc.find("Canvas/UILayer/player2/playerHPBg2");//血量
        this.hpLabel = cc.find("Canvas/UILayer/player2/hpNum2").getComponent(cc.Label);
        ...
        this.angerPro = cc.find("Canvas/UILayer/angerBg2/luqikuang");//怒气
        this.angerLabel = cc.find("Canvas/UILayer/angerBg2/angerNum").getComponent(cc.Label);
    ...
    update(dt) {
        ...
        if (g_defind.winPlayer && g_defind.winPlayer != this.m_ID && Math.abs(this.node.x) < 550) {
            this.node.x += -6 * this.getDirection();
        }
        if (this.blackHpRatio.progress > this.hpRatio.progress) {//暗血6
        ...
        if (!g_defind.winPlayer) {
            ...
            if (g_defind.lock2 && this.attackNum != 0) {//判定第一次出拳
                this.time += dt;
                if (this.time >= 0.5) {
                    this.attackNum = 0;
                    g_defind.lock2 = true;
            ...
            if (this.attackNum != 0 && this.flagAttack) {
                if ((this.collision & 8) == 0 && (this.collision & 4) == 0) {
                    switch (this.attackNum) {
                        case 1:
                            this.node.x += this.getDirection() * 2;
                            break;
                        case 2:
                            this.node.x += this.getDirection() * 2;
                            break;
                        case 3:
                            this.node.x += this.getDirection() * 3;
                            break;
                        case 4:
                            this.node.x += this.getDirection() * 4;
                            break;
                    }
                }
            }
            if (!this.seriousinjuryFlag && this.damageLock) {
                if (this.getDirection() == 1 && (this.collision & 4) == 0) {
                    this.node.x += -this.getDirection() * 4;
                }
                if (this.getDirection() == -1 && (this.collision & 8) == 0) {
                    this.node.x += -this.getDirection() * 4;
                }
                this.node.y += 6;
            }
            if (this.m_State == "standby") {
                if (g_Object.getRole1().getNodeX() < this.node.x) {
                    this.subNode.scaleX = -1;
                } else if (g_Object.getRole1().getNodeX() >= this.node.x) {
                    this.subNode.scaleX = 1;
                }
            }
        }
